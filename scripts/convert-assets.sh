...
export SRC_DIR CACHE_DIR WEBP_QUALITY VP9_CRF OGG_QUALITY PROGRESS_FILE
export IMG_COUNT=$(find "$SRC_DIR" -type f -name "*.png" | wc -l | tr -d ' ')
export VID_COUNT=$(find "$SRC_DIR" -type f -name "*.mp4" | wc -l | tr -d ' ')
export OGG_COUNT=$(find "$SRC_DIR" -type f -name "*.ogg" -size +${OGG_MIN_SIZE}c | wc -l | tr -d ' ')
...
find "$SRC_DIR" -type f -name "*.png" -print0 | \
    xargs -0 -P "$PARALLEL" -n 1 /usr/local/bin/convert-one-image.sh
...
: > "$PROGRESS_FILE"
find "$SRC_DIR" -type f -name "*.mp4" -print0 | \
    xargs -0 -P 4 -n 1 /usr/local/bin/convert-one-video.sh
...
: > "$PROGRESS_FILE"
find "$SRC_DIR" -type f -name "*.ogg" -size +${OGG_MIN_SIZE}c -print0 | \
    xargs -0 -P "$PARALLEL" -n 1 /usr/local/bin/convert-one-audio.sh
